...
{
    public class ComponentCarSpawner : MonoBehaviour
    {
        ...
        private void DetermineNextCars()
        {
            ...
            for(int i = 0; i < count; i++)
            {
                ...
                float x = goingForward ? CarRoadController.rightSideX : CarRoadController.leftSideX;
        ...
        private void SpawnCar(Vector3 position, bool goingForward)
        {
            GameObject prefab;
            if (goingForward)
                prefab = PrefabManager2.Instance.RedCar;
            else
                prefab = PrefabManager2.Instance.BlueCar;
